...
class GeminiAPI {
   ...
   */
  async generateAnswers(questions, userProfile) {
    if (!this.apiKey) {
      throw new Error("Gemini API key not configured");
    }

    const prompt = this.buildPrompt(questions, userProfile);
...
3. For multiple choice questions, select from the provided options only
4. For text questions, provide concise and relevant answers
5. For date questions, use format YYYY-MM-DD if applicable
6. If information is not available in the profile, provide a reasonable default or indicate "N/A"
